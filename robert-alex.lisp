...
(defpackage :robert-alex 
  (:use "COMMON-LISP" "COMMON-LISP-USER")
  (:export "MAKE-COMPUTER-MOVE"))

(in-package :robert-alex)
...
assessment of the game for 'max', from min-wins (-1000) to max-wins (1000)."
  ...
  (let ((mm)
	...
	(thisbeta beta)
	(games))
    ;;if terminal(s) or depth >= maxdepth
    (cond ((or (funcall terminal-p curgame) (>= curdepth depthmax))
	   ;;return eval(s)
	   (return-from alpha-beta (funcall evaluate curgame is-maxs-turn-p)))
	  ;;else if it's max's turn
	  ((funcall is-maxs-turn-p curgame)
	   ;;set games to an array of all child game states  
	   (setf games (funcall expand curgame))
	   ;;loop through child games
	   (loop for child in games
		 ;;recur alpha beta on child
		 do (setf mm (alpha-beta child (incf curdepth) depthmax is-maxs-turn-p expand terminal-p evaluate thisalpha thisbeta))
		    ;;get new alpha
		    (setf thisalpha (max thisalpha mm))
		    ;;if new alpha is greater than beta, return beta
		    (cond ((>= thisalpha thisbeta)
			   (return-from alpha-beta thisbeta))))
	   ;;return new alpha 
	   (return-from alpha-beta thisalpha))
	  ;;else its not max's turn
	  (t
	   ;;set games to an array of all child game states 
	   (setf games (funcall expand curgame))
	   ;;loop through child games
	   (loop for child in games
		 ...
		 do (setf mm (alpha-beta child (incf curdepth) depthmax is-maxs-turn-p expand terminal-p evaluate thisalpha thisbeta))
		    ;;get new beta
		    (setf thisbeta (min thisbeta mm))
		    ;;if alpha is greater than equal to new beta, return alpha
		    (cond ((>= thisalpha thisbeta)
			   (return-from alpha-beta thisalpha))))
	   ;;return new beta
	   (return-from alpha-beta thisbeta)))))
...
if it's max's turn to play."
  (let ((end (game-over game)))
	...
	;;Took most of this code from connectfour.lisp
	;;Just gives points for how many 3 and 2 in a row you have subtrace how many opponent has
	;;Very basic, I know.
	(progn 
	  (let ((width (width game))
		(height (height game))
		(board (board game))
		;;this is used to track how many "almost" situations there are
		(total 0)
		(count)
		;;1 off from finishing a game. This is what we are looking for
		(almost (- (num-in-a-row game) 1)))
		...
		(if (= empty (aref board column row)) (return)  ;; quick bail
			...
			(progn
			  ;;subtract from total
			  (if (= (incf count) almost) (setf total (- total 10)))
			  (if (= count (- almost 1)) (setf total (- total 3))))
			(setf count 0)))))
		  ...
		  (dotimes (row height)
			...
			(if (= empty (aref board column row)) (return) ;; quick bail
				...
				(if (>= (incf (aref board column row)
					      (aref board (1- column) (1- row))) almost)
			...
			(if (= empty (aref board column row)) (return) ;; quick bail
				...
				(if (and (<= (aref board (1- column) (1+ row)) red)
					...
					(progn (reset-board-columns game)
					       ;;add to total
					       (+ total 10))))))))))
	    ;;does logic to decide if you want total or -total
	    (* (turn game) total  (if (funcall is-maxs-turn-p game) -1 1))))
	  ;;; END IMPLEMENTATION
